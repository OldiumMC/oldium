...
public class EntityLighter {
    ...
    public static int getBlendedLight(Entity entity, float tickDelta) {
        ...
        double x1 = MathUtil.lerp(tickDelta, entity.prevX, entity.x);
        double y1 = MathUtil.lerp(tickDelta, entity.prevY, entity.y);
        double z1 = MathUtil.lerp(tickDelta, entity.prevZ, entity.z);
        ...
        for (int bX = bMinX; bX < bMaxX; bX++) {
            ...
            for (int bY = bMinY; bY < bMaxY; bY++) {
                ...
                for (int bZ = bMinZ; bZ < bMaxZ; bZ++) {
                    pos.setPosition(bX, bY, bZ);

                    BlockState blockState = entity.world.getBlockState(pos);

                    // Do not consider light-blocking volumes
                    if (blockState.getBlock().hasTransparency() && blockState.getBlock().getLightLevel() <= 0) {
                        continue;
                    }
                    ...
                    sl += weight * (entity.world.getLightAtPos(LightType.SKY, pos) / MAX_LIGHT_VAL);

                    if (calcBlockLight) {
                        bl += weight * (entity.world.getLightAtPos(LightType.BLOCK, pos) / MAX_LIGHT_VAL);
                    } else {
                        bl += weight;
                    }
                }
